...
# validation variable
iou_loss_threshold = 0.15 # maximum IOU loss for detection prediction to be considered a TP sampl
...
class Dataset(object):
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.open(mask_path).resize(input_size)
        mask_binary = np.where(np.array(mask) != 0, 1, 0)
        masks = mask_binary == [[[1]]]
